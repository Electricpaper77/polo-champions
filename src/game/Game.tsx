...
import { applyBallFieldDrag, BALL_BOUNCE_ELASTICITY, BALL_FIELD_DRAG, BALL_MIN_Y, BALL_SURFACE_FRICTION, canApplyStrike, getBallResetState, getMalletAngle, getMalletSweepContact, getShotImpulse, getStrikePhase } from "./PoloMechanics";
...
function BroadcastPlayfieldOverlay(){const entities=useMatch(s=>s.entities),ball=useMatch(s=>s.telemetry.ball),project=(v:{x:number;z:number})=>({left:`${50+v.x/FIELD_X*88}%`,top:`${50-v.z/FIELD_Z*88}%`}),riderNumber=(id:PoloRiderEntity["id"])=>id==="player"?"3":id.split("_")[1]??"",riderStyle=(rider:PoloRiderEntity)=>({...project({x:rider.position.x,z:rider.position.y}),"--token-heading":`${rider.heading}rad`,transform:`translate(-50%,-50%) rotate(${rider.heading}rad)`}) as CSSProperties;return <section className="playfield-readable-overlay" aria-hidden="true"><div className="field-perspective"><i className="goal goal-north"/><i className="goal goal-south"/><i className="line center-x"/><i className="line center-z"/><i className="line thirty north"/><i className="line thirty south"/>{Object.values(entities).map(rider=><span key={rider.id} className={`field-rider ${rider.team} ${rider.id==="player"?"player":""}`} style={riderStyle(rider)} title={`${rider.team} ${riderNumber(rider.id)}`}><b>{riderNumber(rider.id)}</b><small>{rider.team==="blue"?"B":"R"}</small></span>)}<span className="field-ball" style={project(ball)}/></div></section>}
