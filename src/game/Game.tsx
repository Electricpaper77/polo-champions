...
function BroadcastPlayfieldOverlay(){const entities=useMatch(s=>s.entities),ball=useMatch(s=>s.telemetry.ball),project=(v:{x:number;z:number})=>({left:`${50+v.x/FIELD_X*88}%`,top:`${50-v.z/FIELD_Z*88}%`});return <section className="playfield-readable-overlay" aria-hidden="true"><div className="field-perspective"><i className="goal goal-north"/><i className="goal goal-south"/><i className="line center-x"/><i className="line center-z"/><i className="line thirty north"/><i className="line thirty south"/>{Object.values(entities).map(rider=><span key={rider.id} className={`field-rider ${rider.team} ${rider.id==="player"?"player":""}`} style={project({x:rider.position.x,z:rider.position.y})}>{rider.id==="player"?"3":""}</span>)}<span className="field-ball" style={project(ball)}/></div></section>}
