...
export function PoloEntity({ entity, action = "NONE", motion }: { entity: PoloRiderEntity; action?: EntityActionState; motion?: React.RefObject<{ turn: number; braking: boolean }> }) {
  ...
  useEffect(() => {
    ...
    configurePbr(rider, TEAM_PRESENTATION[entity.team].jersey, true);
  ...
  return <group ref={root} name={`polo-entity-${entity.id}`}>
    ...
    <group ref={riderPivot} position={[0, 1.44, -0.04]}>
      ...
      <mesh position={[0, 1.36, 0]} castShadow><sphereGeometry args={[0.26, 20, 12, 0, Math.PI * 2, 0, Math.PI / 1.7]} /><meshPhysicalMaterial color={presentation.helmet} roughness={0.18} clearcoat={0.75} /></mesh>
