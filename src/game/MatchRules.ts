...
export function resolveRideOffCollision(a:PoloRiderEntity,b:PoloRiderEntity,options:RideOffCollisionOptions={}):RideOffCollisionResult {
  ...
  const normal=distance>.0001?{x:dx/distance,z:dz/distance}:{x:a.id.localeCompare(b.id)<=0?-1:1,z:0};
  const separation=resolveRiderSeparation(a,b,minimum),aContactMass=a.mass*getHorseArchetype(a.archetype).pushResistance,bContactMass=b.mass*getHorseArchetype(b.archetype).pushResistance,totalMass=Math.max(.01,aContactMass+bContactMass);
  const aSpeed=length({x:a.velocity.x,z:a.velocity.y}),bSpeed=length({x:b.velocity.x,z:b.velocity.y});
  const aMomentum=aContactMass*aSpeed*(options.aRideOff?RIDE_OFF_ACTIVE_MULTIPLIER:1);
  const bMomentum=bContactMass*bSpeed*(options.bRideOff?RIDE_OFF_ACTIVE_MULTIPLIER:1);
  ...
  const aShare=bContactMass/totalMass,bShare=aContactMass/totalMass;
