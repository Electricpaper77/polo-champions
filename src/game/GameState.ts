...
export type MatchState = { score:MatchScore; seconds:number; started:boolean; paused:boolean; message:string; activeFoul:ActiveFoul; resetKey:number; entities:Record<PoloRiderEntity["id"],PoloRiderEntity>; telemetry:Telemetry; scoreGoal:(team:MatchTeam)=>void; resetBall:()=>void; togglePause:()=>void; restart:()=>void; setStarted:(started:boolean)=>void; setSeconds:(n:number)=>void; setMessage:(s:string)=>void; setActiveFoul:(foul:ActiveFoul)=>void; setEntities:(entities:Record<PoloRiderEntity["id"],PoloRiderEntity>)=>void; setTelemetry:(telemetry:Telemetry)=>void };
...
export const useMatch = create<MatchState>((set) => ({
  score:{blue:0,red:0}, seconds:420, started:false, paused:false, message:"KICK OFF · MOVE TO START", activeFoul:null, resetKey:0, entities:initializeMatchEntities(), telemetry,
  scoreGoal:(team)=>set(s=>({score:{...s.score,[team]:s.score[team]+1},started:false,message:`${team.toUpperCase()} GOAL!`,activeFoul:null,entities:initializeMatchEntities(),resetKey:s.resetKey+1})),
  resetBall:()=>set(s=>({started:false,resetKey:s.resetKey+1,message:"BALL RESET · MOVE TO START",activeFoul:null,entities:initializeMatchEntities()})),
  togglePause:()=>set(s=>({paused:!s.paused,message:!s.paused?"PAUSED":s.started?"PLAY":"KICK OFF · MOVE TO START"})),
  restart:()=>set(s=>({score:{blue:0,red:0},seconds:420,started:false,paused:false,message:"KICK OFF · MOVE TO START",activeFoul:null,entities:initializeMatchEntities(),telemetry,resetKey:s.resetKey+1})),
