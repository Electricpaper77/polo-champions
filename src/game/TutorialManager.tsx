...
export function TutorialOverlay({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => { window.POLO_TUTORIAL_ACTIVE = true; window.dispatchEvent(new Event("polo-tutorial-mode")); return () => { window.POLO_TUTORIAL_ACTIVE = false; window.dispatchEvent(new Event("polo-tutorial-mode")); }; }, []);
