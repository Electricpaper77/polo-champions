...
import { PlayerProfileStore, XP_PER_LEVEL } from "../services/PlayerProfile";
...
export function MainMenu({ user, onQuickMatch, onPartyLobby }: { user: PlatformUser | null; onQuickMatch: () => void; onPartyLobby: () => void }) {
  ...
  const localProfile = PlayerProfileStore.get();
  const [shareStatus, setShareStatus] = useState("");
  return <main className="lobby" aria-label="British Polo Multiplayer lobby">
    <header className="profile-header" aria-label="Player profile">{user ? <><div className="profile-badge">PC</div><div><strong>{user.displayName}</strong><small>LEVEL {localProfile.level}</small></div><span className="xp"><i style={{ width: `${localProfile.xp / XP_PER_LEVEL * 100}%` }} /></span><small>{localProfile.xp}/{XP_PER_LEVEL} XP</small><div className="currencies"><b>◆ {user.gold} GOLD</b><b>● {localProfile.poloCoins} POLO COINS</b></div></> : <span>LEVEL {localProfile.level} · {localProfile.xp}/{XP_PER_LEVEL} XP</span>}</header>
