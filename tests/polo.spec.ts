...
async function enterMatch(page: import("@playwright/test").Page) { await page.getByRole("button", { name: "PLAY QUICK MATCH" }).click(); await expect(page.getByText("SEARCHING FOR MATCH")).toBeVisible(); await expect(page.locator("canvas")).toBeVisible({ timeout: 8_000 }); }
